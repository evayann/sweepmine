...
const generateCase = (x: number, y: number): Case => ({ position: new Vector2(x, y), isBomb: false, numberOfBombsArround: 0, isReveal: false });
...
export class MineSweeper {
    ...
    revealCase(x: number, y: number, exceptCaseList: Vec2[] = []): MineSweeper {
        ...
        this.revealNeighbours(x, y, exceptCaseList);
    ...
    private revealNeighbours(x: number, y: number, exceptCaseList: { x: number, y: number }[]): void {
        if (this.hasBombAsNeighbour(x, y)) return;

        const neigbourList = this.getNeighbourListOf(x, y);
        neigbourList
            .filter(neigbourCase => !exceptCaseList.includes(neigbourCase.position))
            .forEach(neigbourCase => {
